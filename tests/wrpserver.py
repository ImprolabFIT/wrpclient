...
class WRPServer:
    ...
    async def run(self):
        ...
        self.server_task = asyncio.create_task(
            self.__handle_server_exception__())
    ...
    def handle_message(self, message):
        ...
        if(message.msg_type == Message.Type.GET_CAMERA_LIST):
            if(self.state == WRPServer.State.CONNECTED):
                to_send = Message.create_message(
                    Message.Type.CAMERA_LIST,
                    xml_camera_list=WRPServer.camera_list_to_xml(
                        self.camera_list))
            else:
                to_send = Message.create_message(
                    Message.Type.ERROR, error_code=Message.ErrorCode.UNEXPECTED_MESSAGE)

        elif(message.msg_type == Message.Type.OPEN_CAMERA):
            if(self.state == WRPServer.State.CONNECTED):
                serial_number = getattr(
                    message, Message.CAMERA_SERIAL_NUMBER_ATTR_NAME)
                try:
                    camera = [
                        c for c in self.camera_list if c.SerialNumber == serial_number][0]
                    self.state = WRPServer.State.CAMERA_SELECTED
                    self.camera_selected = camera
                    to_send = Message.create_message(Message.Type.OK)
                except IndexError:
                    to_send = Message.create_message(
                        Message.Type.ERROR, error_code=Message.ErrorCode.CAMERA_NOT_FOUND)
            else:
                to_send = Message.create_message(
                    Message.Type.ERROR, error_code=Message.ErrorCode.UNEXPECTED_MESSAGE)
            ...
            if(self.state == WRPServer.State.CAMERA_SELECTED):
                ...
                to_send = Message.create_message(
                    Message.Type.FRAME, **self.frame_dict)
            else:
                to_send = Message.create_message(
                    Message.Type.ERROR, error_code=Message.ErrorCode.UNEXPECTED_MESSAGE)
        else:
    ...
    async def receive_message(reader):
        ...
        print(
            f"Server: Received message type value: {message_type_value}, payload_length: {payload_length}")
        if(payload_length > 0):
    ...
    def camera_list_to_xml(camera_list):
        ...
        for cam in camera_list:
            ET.SubElement(root, "Camera", **{key: str(val)
                                             for key, val in vars(cam).items()})
